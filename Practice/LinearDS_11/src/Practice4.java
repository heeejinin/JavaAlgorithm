...
class MyDeque2 {
    ...
    public void increaseSize() {
        ...
        int[] addTmp = this.arr.clone();
        this.arr = new int[this.arr.length * 2];

        int start = (this.front + 1) % addTmp.length;
        int end = (this.rear + 1) % addTmp.length;

        int idx = 1;
        for (int i = start; i != end; i = (i + 1) % addTmp.length) {
            this.arr[idx++] = addTmp[i];
        }
